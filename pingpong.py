...
class Enemy():
    def __init__(self, pong_sound):
        self.rect = pygame.Rect(SCREEN_WIDTH//2, 2, 40, 50)
    ...
    def update(self, ball):
        ...
        #적이 공 왼쪽에 있을때
            ...
            diff = ball.rect.centerx - ball.rect.centerx
            if diff > 4:
                self.rect.x += 4
            else:
                self.rect.centerx = ball.rect.centerx
...
class Game():
    def __init__(self):
        bounce_sound = pygame.mixer.Sound(os.path.join(assets_path, "bounce_wav"))
        pong_sound = pygame.mixer.Sound(os.path.join(assets_path, "ping_wav"))
        ping_sound = pygame.mixer.Sound(os.path.join(assets_path, "pong_wav"))
    ...
    def run_logic(self):
        ...
        if self.ball.top[1] < 0:
            self.player_score += 1
            self.ball.reset(salf.player.rect.centerx, self.player.rect.centery)

        #공이 아래로 나간 경우(enemy 승)
        if self.ball.reect.top[1] < 0:
            self.player_score += 1
            self.ball.reset(self.enemy.rect.centerx, self.player.rect.center)
    ...
    def display_fame(self, screen):
        ...
        if self.player_score == 10:
            ...
            enemy_score_label = self.font.render(str(self.enemy_scoer), True, WHITE)
            screen.blit(enemy_score_label, (10, 260))
            player_score_label = self.font.render(str(self.enemy_scoer), True, WHITE)
